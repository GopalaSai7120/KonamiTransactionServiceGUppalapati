...
public class AdjustOperation {
     ...
     *   sequence number , DEPOSIT transaction type
     * 6)if all conditions are met proceed with Inserting DEPOSIT_VOID Transaction
     * */
    CommonOperations commonOperations = new CommonOperations();

    public void adjust(TransactionTypeEnum transactionTypeEnum, Integer amount, Integer personId, Integer transactionSequence) throws InvalidInputException {
        try {
            ...
            checkifTransactionSequenceIsvalidforWithdraw(transactionSequence);

            Integer transactionId = HeapMemoryStorage.TRANSACTION_TABLE.size();
            TransactionTable transactionTable = new TransactionTable(100 + transactionId, TransactionTypeEnum.ADJUST, amount, personId, transactionSequence);
            HeapMemoryStorage.TRANSACTION_TABLE.add(transactionTable);
            System.out.println("Operation Success: PERSON_ID:" + personId + " has access to perform ADJUST operation");
    ...
    private void checkifTransactionSequenceIsvalidforWithdraw(Integer transactionSequence) throws DataInvalidException {
