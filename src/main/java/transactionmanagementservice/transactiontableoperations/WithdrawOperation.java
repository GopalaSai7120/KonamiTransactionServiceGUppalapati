...
public class WithdrawOperation {
     ...
     * 4)Verify if Person has access to perform WITHDRRAW transaction in PERSON_CAN_DO Table
     * 5)if all conditions are met proceed with Inserting DEPOSIT_VOID Transaction
     * */

    CommonOperations commonOperations = new CommonOperations();

    public void withDraw(TransactionTypeEnum transactionTypeEnum, Integer amount, Integer personId, Integer transactionSequence) throws InvalidInputException {
        try {
            ...
            TransactionTable transactionTable = new TransactionTable(100 + transactionId, TransactionTypeEnum.WITHDRAW, amount, personId, transactionSequence);
